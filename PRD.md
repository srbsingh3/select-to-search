...
- FR8 Styling baseline: Barebones CSS with semantic class names; define tokens in `:host` for Shadow DOM (`--space-*`, `--font-size-*`, `--radius-*`, `--color-*`). Content script styles are inlined in Shadow DOM for isolation; `options.css` for settings page.
