...
- FR5 Settings (options page): React + Vite single-page options UI that lets users: (a) enable/disable extension, (b) choose affordance mode (quick actions vs picker), (c) configure which quick-action buttons are shown (single = default provider, dual default ChatGPT+Google, or all three with Claude off by default), (d) edit optional prefix. Persist via `chrome.storage.sync`; load on start and reflect current values.
